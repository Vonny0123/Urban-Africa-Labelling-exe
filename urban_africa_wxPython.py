...
class UAFrame(wx.Frame):
    ...
    def __init__(self):
        '''
        Sets out the initial layout of the window.
        '''
        ...
        st_description = wx.StaticText(self.pnl, label="This application will label entries in a dataset as urban or rural based on latitude and longitude using the Africapolis dataset.")
    ...
    def upload_data(self, event):
        '''
        Loads selected data into memory and reveals the next steps in the window.
    ...
    def process_africapolis(self):
        '''
        Processes data from Africapolis to get a function that checks if a point lies in any city polygon in Africa.
    ...
    def multi_process_containment_tests(self):
        ...
        with cb:
            is_urban = dd.from_pandas(points.geometry, npartitions=100).map_partitions(lambda dframe: pd.Series(np.any(dframe.apply(containment_checker), axis=1)), meta=pd.Series(dtype=bool)).compute(scheduler='processes')  
        return is_urban
    ...
    def validate_countries(self, event):
        ...
        ----------
        event : Button click event
            Checks that entries in the selected 'Country' column are valid according to the Africapolis dataset. If any country is not valid, the user is asked which valid country it refers to.
    ...
    def filter_countries(self, event):
        '''
        Filters the Africapolis data so it only includes countries relevant to the user.

        Parameters
        ----------
        event : Button click event
            The filter button clicked.
    ...
    def OnBeginClick(self, event):
        '''
        Implements the labelling tool with the inputs given when the user clicks on the final button.
    ...
    def OnFiltClick(self, event):
        '''
        Rearranges the window based on whether the user wishes to filter for relevant countries or not.

        Parameters
        ----------
        event : Checkbox changed event
            The country filter check box state was changed.
    ...
    def ProgCallback(self, key, result, dsk, state, id):
        '''
        A function provided to Dask as a callback, after each chunk of work is completed, the progress bar is updated.
        '''
        self.prog += 1
